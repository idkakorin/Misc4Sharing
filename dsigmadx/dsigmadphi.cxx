...
{
public:
  d3XSecMK_dWQ2CosTheta_E(const XSecAlgorithmI * m, const Interaction * i, double phi, double Wcut);
...
int main(int argc, char ** argv)
{
  ...
  double xsec, enu = 1., phi = 0.5, Wcut = 1.4;;
  ...
  if (phi < 0 || phi > 2*kPi) return -1;
  
  interaction->InitStatePtr()->SetProbeP4(p4);
  
  d3XSecMK_dWQ2CosTheta_E func(alg, interaction, phi, Wcut ) ; 
...
d3XSecMK_dWQ2CosTheta_E::d3XSecMK_dWQ2CosTheta_E(
								    const XSecAlgorithmI * m, const Interaction * interaction, double phi, double Wcut) :
...
double d3XSecMK_dWQ2CosTheta_E::DoEval(const double * xin) const
{
  ...
  fInteraction->KinePtr()->SetKV(kKVctp, -1. + 2.*xin[2]);
...
{
  return
    new d3XSecMK_dWQ2CosTheta_E(fModel,fInteraction,fphi, fWcut);
}
